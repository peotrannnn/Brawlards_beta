...
export class GuyAI {
    resetState() {
        ...
        this.isGhost = false;
        this.currentPhase = 1;
        // Reset body mass nếu bị ghost mode
        if (this.body) {
  ...
  update(delta, syncList) {
        ...
        // Nếu không có player hợp lệ thì không di chuyển, không ghost mode
        if (!nearestPlayer || !nearestPlayer.mesh || !nearestPlayer.body) {
            // Reset ghost mode nếu đang ở phase 6
            if (this.isGhost) {
                this.isGhost = false;
                this.body.mass = this.originalMass;
                this.body.updateMassProperties();
        ...
        const playerVisible = minDistance <= this.visionRange;
        // GHOST MODE
        if (phase === 6) {
            if (!this.isGhost) {
                this.isGhost = true;
        ...
        if (this.jumpCooldown <= 0 && this.canJump && this.isGrounded) {
            ...
            const randomOffset = (Math.random() - 0.5) * 2 * currentJumpInterval;
        ...
        const futurePos = new THREE.Vector3().copy(playerPos).addScaledVector(nearestPlayer.body.velocity, 0.5);
        ...
        if (!this.isReversing) {
            ...
            if (this.randomMoveTimer <= 0) {
                this.randomMoveTimer = 1 + Math.random() * 2 + (1 - Math.min(1, this.timeAlive / 60)) * 2;
                this.randomDir.set(Math.random() - 0.5, 0, Math.random() - 0.5).normalize();
            }
        ...
        if (this.isReversing) {
            ...
            moveDir.set(-Math.sin(this.bodyYaw), 0, -Math.cos(this.bodyYaw));
        ...
        if (this.stuckTimer > 0.6) {
            ...
            this.body.velocity.x = 0;
            this.body.velocity.z = 0;
            this.escapeIndex = (this.escapeIndex + 1) % this.escapeAngles.length;
            this.escapeAttempts++;
